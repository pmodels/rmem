...
#define m_gettime(t) clock_gettime(CLOCK_MONOTONIC, (t))
#define m_get_wtimes(t0, t1) \
    ((double)((t1).tv_sec - (t0).tv_sec) * 1.0e+6 + (double)((t1).tv_nsec - (t0).tv_nsec) * 1.0e-3)
...
static inline void rmem_prof_start(rmem_prof_t* prof) { m_gettime(&prof->t0); }
static inline void rmem_prof_stop(rmem_prof_t* prof, double* time) {
    m_gettime(&prof->t1);
    *time = m_get_wtimes(prof->t0, prof->t1);
}

#define m_dfr(begin, end)       for (int m_dfr_i = (begin, 0); !m_dfr_i; (m_dfr_i += 1, end))
#define m_rmem_prof(prof, time) m_dfr(rmem_prof_start(&prof), rmem_prof_stop(&prof, &time))
