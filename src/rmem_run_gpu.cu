...
void cuda_trigger_op(const size_t n_msg, int* data, const size_t len, rmem_gpu_op_t op,
                     void* op_array, void* op_data, gpuStream_t stream) {
    switch (op) {
        case RMEM_GPU_PUT: {
            cuda_trigger_op_device_put<<<1, 1, 0, stream>>>(n_msg, data, len);
        } break;
        case RMEM_GPU_P2P: {
            cuda_trigger_op_device_p2p<<<1, 1, 0, stream>>>(drma);
        } break;
    }
}
